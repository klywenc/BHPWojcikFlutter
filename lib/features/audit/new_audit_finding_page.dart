...
const _departments = <String>[
  'Produkcja', 'Magazyn', 'Biuro', 'Utrzymanie ruchu',
];

const _categories = <String>[
  'BHP', 'Jakość', 'Wypadek', 'Awaria', 'Inne',
];

class NewAuditFindingPage extends StatefulWidget {
  const NewAuditFindingPage({super.key});

  @override
  State<NewAuditFindingPage> createState() => _NewAuditFindingPageState();
}

class _NewAuditFindingPageState extends State<NewAuditFindingPage> {
  ...
  bool get _canPreview =>
      _department != null && _descCtrl.text.trim().isNotEmpty && _photos.isNotEmpty;
  ...
  Future<void> _goPreview() async {
    ...
    if (!_canPreview) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
        content: Text('Uzupełnij dział, opis i dodaj co najmniej jedno zdjęcie.'),
      ));
      return;
    }
...
class _PhotosGrid extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final items = <Widget>[
      ...
      _AddTile(icon: Icons.photo_camera_outlined, label: 'Aparat', onTap: onAddFromCamera),
      _AddTile(icon: Icons.add_photo_alternate_outlined, label: 'Galeria', onTap: onAddFromGallery),
    ];
